...
import ratpack.server.core.http.HttpUrlBuilder;
...
public interface TestHttpClient {
   ...
   *
   * @param applicationUnderTest the Ratpack application to make requests against
   * @param requestConfigurer a {@link ratpack.func.Action} that will set up the {@link RequestSpec} for all requests made through this instance of TestHttpClient
   * @return a http client which is configured to make requests against the provided ApplicationUnderTest
   */
  ...
  /**
   * @param requestAction an {@link ratpack.func.Action} that will act on the {@link RequestSpec} this is used to configure details of the next request
   * @return this
   */
