...
package ratpack.server.core.http.client
...
import static ratpack.server.core.http.ResponseChunks.stringChunks
import static ratpack.server.core.http.internal.HttpHeaderConstants.CONTENT_ENCODING
...
class HttpClientSmokeSpec extends BaseHttpClientSpec {
  ...
  def "can set read timeout from pooling config"() {
    ...
    then:
    text == "ratpack.server.core.http.client.HttpClientReadTimeoutException: Read timeout (PT1S) waiting on HTTP server at $otherApp.address".toString()
