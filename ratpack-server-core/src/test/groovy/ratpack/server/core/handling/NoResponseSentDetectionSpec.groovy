...
class NoResponseSentDetectionSpec extends RatpackGroovyDslSpec {
  ...
  def "message contains custom class name"() {
    ...
    then:
    getText() == "No response sent for GET request to / (last handler: ratpack.server.core.handling.NoResponseSentDetectionSpec\$MyHandler)"
    response.statusCode == 500
  }
  ...
  def "message contains custom inner class name"() {
    ...
    then:
    getText() == "No response sent for GET request to / (last handler: anonymous class ratpack.server.core.handling.NoResponseSentDetectionSpec\$1 at approximately line 81 of NoResponseSentDetectionSpec.groovy)"
    response.statusCode == 500
  }
  ...
  def "message contains custom inner class name when class is empty"() {
    ...
    then:
    getText() == "No response sent for GET request to / (last handler: ratpack.server.core.handling.NoResponseSentDetectionSpec\$2)"
    response.statusCode == 500
  }
